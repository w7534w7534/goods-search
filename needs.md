...
- **資料來源**：
  ...
  - **即時**：TWSE / TPEX Open Data（盤中即時報價，約 20 秒延遲，10 秒快取）
...
| FR-6.4 | 即時報價 | 盤中自動輪詢（15s 價格 / 60s 指標），含即時 K 線 + 技術指標重算，自動判斷上市/上櫃，交易時段自動啟停 | ✅ 已完成 |
...
- API 快取：收盤資料 TTL 5 分鐘，即時報價 TTL 10 秒，股票清單每日更新
- 前端圖表載入須 < 3 秒（一般網路）
- 前端分批載入：優先渲染 K 線圖，其餘區塊非同步載入
- 盤中即時更新：價格 15 秒 / 技術指標 60 秒輪詢
...
### P1 — 功能增強
1. ~~新增技術指標：BIAS、ATR~~ ✅ 已完成
2. ~~籌碼面：券資比、外資持股趨勢圖、主力連買天數~~ ✅ 已完成
3. ~~基本面：ROE/ROA、負債比、配息率~~ ✅ 已完成（FCF 待開發）
4. 估值河流圖 (P/E Band, P/B Band)
5. ~~買賣訊號標記（黃金交叉/死亡交叉/MACD 翻多等）~~ ✅ 已完成
6. 個股筆記 + 操作紀錄
7. 新增技術指標：PSY、SAR、CCI

### P2 — 體驗升級
1. ~~即時報價（TWSE/TPEX API + 自動刷新 + 即時指標重算）~~ ✅ 已完成
2. 多股比較功能
3. PWA 支援
4. 搜尋歷史 + 動態熱門排行
5. Excel 匯出
...
| 問題 | 修正內容 |
|:--|:--|
| VWAP 累計失真 | 改為 20 日滾動計算 |
| PER 區間不足 | 預設查詢從 1 個月改為 3 個月 |
| 前端日期 UTC 偏移 | `toISOString()` 改為本地時間格式化 |
| 股利固定起始日 | `2015-01-01` 改為動態回溯 10 年 |
| K 線 tooltip OHLC 偏移 | ECharts category index 導致開盤價顯示為索引值 |
| TWSE SSL 憑證失敗 | Python 3.14 嚴格驗證問題，加 `verify=False` 繞過 |